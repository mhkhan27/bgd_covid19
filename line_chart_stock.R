...
final_group_gather <- final %>% group_by(key,round) %>% summarise(
  ...
  final_data_for_chart <- final_group_gather %>% dplyr::mutate(
    ...
    color = if_else(grepl("rice",key),"#d0cdbc",
                                            ...
                                            if_else(grepl("eggs",key),"#d3e5d4",
                                                    if_else(grepl("bananas",key),"#53b5cd",
                                                            if_else(grepl("fish",key),"#eeadad",
                                                                    if_else(grepl("chicken",key),"#ee5658",
                                                                            if_else(grepl("soap",key),"#BCF5A9",
                                                                                    if_else(grepl("powder",key),"#F3E2A9",
                                                                                            if_else(grepl("paracetamol",key),"#A9BCF5",
                                                                                                    if_else(grepl("tarpaulin",key),"#CEF6CE","error",NULL
                                                                    )))))))))))))
  ...
  ggsave(path = outputfolder_box,filename ="line_stock_item.jpg" ,width=12.9213,height=7,units="cm",scale = 1.8)
